 ...
 */
    ...
    func insertNewObject(_ sender: Any) {
        var task = URLSession(configuration: URLSessionConfiguration.default).streamTask(withHostName: "127.0.0.1", port: 7000)
        task.resume()
        
        task.readData(ofMinLength: 0, maxLength: 8, timeout: 0) {(data, response, erorr) in
            print(data)
